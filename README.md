...
<<<<<<< HEAD
    （2）常用的GIT指令：git init;git add .;git commit -m "update";git push;git pull;git remote add 远程仓库地址;git remote remove 远程仓库地址;git status;git config user.name;git config user.email;git remote -v;git rm -r --cached node_modules/
=======
  （2）常用的GIT指令：
        git init;
        git add .;
...
>>>>>>> 15b39087412b72d6a1e6467c37bae977033121ac
六、项目打包部署流程（前端开发流程）
